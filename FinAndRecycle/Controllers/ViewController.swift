...
class ViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate {
    ...
    @IBOutlet weak var collectionView: UICollectionView!
        ...
        // Pedometer property is no longer strictly needed if always hardcoding
        // let pedometer = CMPedometer() // You can comment out or remove this

        // API key for accessing the News API - Remember to keep API keys secure in real apps!
        private let apiKey = "f869c8bcd91543ac9b9689504470c0be" // Replace with your actual API key
        ...
        override func viewDidLoad() {
            ...
            // --- Set Hardcoded Step Value ---
            let hardcodedSteps = 7500
            // Format the number for better readability (optional)
            let numberFormatter = NumberFormatter()
            numberFormatter.numberStyle = .decimal
            let formattedSteps = numberFormatter.string(from: NSNumber(value: hardcodedSteps)) ?? "\(hardcodedSteps)"
            stepCountLabel.text = "\(formattedSteps) Steps Today"
            // ---------------------------------

            // Comment out or remove the call to fetch real steps from CMPedometer
        ...
        // NOTE: FeedCell class remains unchanged
        ...
        // NOTE: DetailViewController class remains unchanged
        ...
        // MARK: - Pedometer Functions (Commented Out or Remove)
        /*
             ...
             pedometer.queryPedometerData(from: startOfDay, to: now) { [weak self] (pedometerData, error) in
                 ...
                 DispatchQueue.main.async {
                     ...
                     if let data = pedometerData {
                         ...
                         // Format the number for locale-specific display
                        let numberFormatter = NumberFormatter()
                        numberFormatter.numberStyle = .decimal
                        let formattedSteps = numberFormatter.string(from: NSNumber(value: steps)) ?? "\(steps)"
                        self.stepCountLabel.text = "\(formattedSteps) Steps Today"
